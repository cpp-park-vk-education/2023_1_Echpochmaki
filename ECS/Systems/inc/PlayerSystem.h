...
protected:
    void controlEntity(Entity *entity) {
        if (entity->HasComponent<VelocityComponent>()) {
            int dir = 0; //Handling the direction

            std::cout << "[playersys] window->hasFocus()=" << window->hasFocus() << std::endl;
...
//            auto& attack = entity->getComponent<AttackAnimationComponent>();
            ...
            if ((window == nullptr || window->hasFocus()) && Mouse::isButtonPressed(Mouse::Left)) {
